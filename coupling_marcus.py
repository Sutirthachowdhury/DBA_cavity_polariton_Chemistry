...
for g in range(param.nstep):
    ...
    for n in range(param.nfock):
        for m in range(param.nfock):
            for l in range(param.nbridge):
                vda[n,m,g] = vda[n,m,g] - 0.5*vdb[n,l]*vba[l,m]*((1.0/((param.dE+dg)+(l-m)*param.omega_c)) \
...
for g in range(param.nstep):
     ...
     delg = dg*(((param.diab_BA)**2/((param.dE+dg)*param.dE))-1.0) 
...
f = open("rate_vs_dg_dE_1.5ev_LM_1mev_omega200mev.txt","w+")
